...
export default function CategoriesExplorerPage() {
  ...
  // Sort categories based on numerical values
  const sortedCategories = [...SPENDING_CATEGORIES].sort((a, b) => {
  ...
  return (
    <div className="space-y-10 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-blue-700 px-3 py-1 rounded bg-blue-50 border border-blue-200 mb-2">
            ...
            Verified Federal Functions
          </div>
          <h1 className="text-3xl font-black text-slate-900">
            U.S. Government Spending Categories
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Explore official U.S. federal budget functions and mandatory benefit programs for FY{selectedFY}.
          </p>
        </div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 space-y-1">
                  ...
                  <div className="text-xs text-slate-600 font-medium">
                    <strong className="text-slate-900">{c.percentage}%</strong> of selected total
                  </div>
                  <div className="text-[11px] text-slate-500 font-mono pt-1">
                    Estimated rate: <span className="font-bold text-slate-800">{hourlyRateFormatted} / hour</span>
                  </div>
                </div>
              </div>
      ...
      <SpendingBreakdown data={sortedCategories} />
    </div>
  );
}
